...
export default class PlaylistThunks {
            ...
            return PlaylistRequests.getAll().then(response => {
                if (response.status == 200) {
                    response
                        .json()
                        .then(json =>
                            dispatch(PlaylistPureActions.returnAll(json))
                        );
                } else {
            ...
            return PlaylistRequests.getArtwork({ playlist }).then(response => {
                if (response.status == 200) {
                    response.blob().then(blob => {
                        dispatch(
                            PlaylistPureActions.returnArtwork(playlist, blob)
                        );
                    });
                } else {
                    dispatch(
                        PlaylistPureActions.getArtworkFailure(
                            playlist,
                            response
                        )
                    );
        ...
        return dispatch => {
            ...
            return PlaylistRequests.add({ playlist }).then(response => {
                if (response.status == 200) {
                    ...
                    dispatch(
                        PlaylistPureActions.addFailure(playlist, response)
                    );
                }
            });
        };
        ...
        return dispatch => {
            ...
            return PlaylistRequests.delete({ playlist }).then(response => {
                if (response.status == 200) {
                    ...
                    dispatch(
                        PlaylistPureActions.deleteFailure(playlist, response)
                    );
                }
            });
        };
    ...
    static deleteSong(playlist, song) {
        return dispatch => {
            ...
            return PlaylistRequests.deleteSong({
                playlist,
                song
            }).then(response => {
                if (response.status == 200) {
                    dispatch(
                        PlaylistPureActions.deleteSongSuccess(playlist, song)
                    );
                } else {
                    dispatch(
                        PlaylistPureActions.deleteSongFailure(
                            playlist,
                            song,
                            response
                        )
    ...
    static addSong(playlist, song) {
        return dispatch => {
            ...
            return PlaylistRequests.addSong({
                playlist,
                song
            }).then(response => {
                if (response.status == 200) {
                    dispatch(
                        PlaylistPureActions.addSongSuccess(playlist, song)
                    );
                } else {
                    dispatch(
                        PlaylistPureActions.addSongFailure(
                            playlist,
                            song,
                            response
                        )
