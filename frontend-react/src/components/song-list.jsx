...
class SongList extends React.Component {
    constructor() {
        ...
        this.toggleAddingSong = this.toggleAddingSong.bind(this);
    }

    componentWillReceiveProps(nextProps) {
        if (this.props.currentPlaylist != nextProps.currentPlaylist) {
            this.props.dispatch(
                SongThunks.getAllPlaylist(nextProps.currentPlaylist)
            );
        }
    }
    ...
    componentWillMount() {
        this.props.dispatch(
            SongThunks.getAllPlaylist(this.props.currentPlaylist)
        );
    }
    ...
    selectChange(selection) {
        this.props.dispatch(
            PlaylistThunks.addSong(this.props.currentPlaylist, selection.value)
        );
        this.toggleAddingSong();
    }

    toggleAddingSong() {
        this.setState(prevState => ({
            addingSong: !prevState.addingSong
        }));
    }
    ...
    render() {
        ...
        if (
            ...
            this.props.songs.data['All Songs']
        ) {
            options = this.props.songs.data['All Songs'].map(s => {
                return {
                    value: s.Path,
